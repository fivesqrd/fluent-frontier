Fluent for PHP
============
Fluent generates beautiful and responsive transactional emails quickly with minimal markup. It's great for user notifications like password resets, user welcomes, receipts, shipping notifications, etc.
...
## How it works ##
At the foundation is a single column responsive layout with support for several types of UI components. Each UI component occupies the full width of the layout and the content is built up by stacking one component on top of another. By combining several UI components together, one can easily generate many of the most common types of user notifications needed in a web or mobile app. 
...
## Install ##
To make use of Fluent Server, you'll need a [Fluent Server](http://fluentmsg.com) account and the Fluent Client:

```
composer require fluent/client:4.0
```

The following config can be preloaded during your app's bootstrap process:
```
$config = [
...
## Template Classes ##
Generate from a pre-built message template
```
...
In some cases one may need to send content not generated by Fluent. The send() method can be used
to simply deliver pre-built content: 
```
$messageId = (new Fluent\Message())->send([
    'content'   => $content,
    'subject'   => 'Testing it', 
